  ...
  if (isModernAPI) {
    ...
    sassOptions.loadPaths = []
      .concat(
        ...
        (sassOptions.loadPaths ? sassOptions.loadPaths.slice() : []).map((includePath) =>
          _path.default.isAbsolute(includePath) ? includePath : _path.default.join(process.cwd(), includePath),
        ),
      )
      .concat(process.env.SASS_PATH ? process.env.SASS_PATH.split(process.platform === 'win32' ? ';' : ':') : []);
    ...
    sassOptions.importer = sassOptions.importer
      ? proxyCustomImporters(
          Array.isArray(sassOptions.importer) ? sassOptions.importer.slice() : [sassOptions.importer],
          loaderContext,
        )
      : [];
    ...
    sassOptions.includePaths = []
      ...
      .concat(
        ...
        (sassOptions.includePaths ? sassOptions.includePaths.slice() : []).map((includePath) =>
          _path.default.isAbsolute(includePath) ? includePath : _path.default.join(process.cwd(), includePath),
        ),
      )
      .concat(process.env.SASS_PATH ? process.env.SASS_PATH.split(process.platform === 'win32' ? ';' : ':') : []);
